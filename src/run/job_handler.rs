...
impl JobHandler{
    ...
    pub fn submit_jobs(&self, experiment_path: &str, benchmark_path: &str, dry_run: &bool) -> Result<(), std::io::Error> {
        ...
        let mut repl_maps = parser.get_arguments();

        if ! dry_run {
            //Obtain a unique hash from the server
            let mut hashes = ssh::get_hash_titan(repl_maps.benchmark_suites.len())?.into_iter();

            for benchmark_suite in &mut repl_maps.benchmark_suites {
                self.submit_one_job(benchmark_suite, &hashes.next().unwrap())?;
            }
        }

        let dst = parser.get_exp_dst();
        let _ = write_submit_job_map(&repl_maps, &dst);

        Ok(())
    }

    fn submit_one_job(&self, benchmark_suite: &mut BenchmarkSuite, hash: &str) -> Result<(), std::io::Error> {
        ...
        if let Some(job_nr) = self.hpc_handler.submit_job(&job_file_path)? {
            ...
            //Now creat the experiment files
            let exp_dir = self.create_job_files(benchmark_suite, &job_nr)?;
            self.hpc_handler.submit_experiment(&exp_dir);

            benchmark_suite.job_nr = Some(job_nr.to_owned());
            benchmark_suite.change_state_benchmarks(&None, &JobStatus::SUBMITTED);
    ...
    fn create_job_files(&self, job_argument: &mut BenchmarkSuite, job_nr: &str) -> Result<PathBuf, std::io::Error> {
        let temp_exp_path = self.temp_path.join(Path::new(&job_nr));
        if ! temp_exp_path.is_dir() { let _ = fs::create_dir(&temp_exp_path); }

        let mut task_idx = 1;
        for SimulatorParameter{sniper_dir_name: _, variable_sniper_parameters: _, benchmark_parameters} in &mut job_argument.simulator_parameters{
            for BenchmarkParameter{arguments, benchmark_name: _, run_idx: _, task_idx: benchmark_task_idx, status: _} in benchmark_parameters {
                    ...
                    match(benchmark_suite.job_nr.as_ref(), benchmark_parameter.task_idx){
                        (Some(job_nr), Some(task_idx)) => {
                            ...
                            if ! titan_job_ids.contains(&bench_job_id) {
                                let dst_path = benchmark_suite.host_dst_path.join(&simulator_parameter.sniper_dir_name);
...
pub fn write_submit_job_map(job_arguments: &Experiment, host_dst_path: &Path) -> std::io::Result<()> {
